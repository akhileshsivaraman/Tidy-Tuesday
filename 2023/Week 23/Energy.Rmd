...
## How is energy consumption around the world changing?
```{r}
...
ggplot(world_energy_cons_change_pct) +
  ...
  ggtitle("Annual percentage change in primary energy consumption across the world")
```

The top 5 consumers of energy produced by fossil fuels in 2021 were:
```{r echo=TRUE, include=TRUE}
top_consumers <- owid_energy |>
  filter(year == 2021) |>
  arrange(desc(fossil_fuel_consumption)) |>
  drop_na(iso_code) |>
  pull(country)
top_consumers[1:5]
```
But per capita, the top 5 consumers of energy produced by fossil fuels in 2021 were:
```{r echo=TRUE, include=TRUE}
top_consumers_capita <- owid_energy |>
  filter(year == 2021) |>
  arrange(desc(fossil_energy_per_capita)) |>
  drop_na(iso_code) |>
  pull(country)
top_consumers_capita[1:5]
```

Using the tools below, we can firstly observe how energy consumption in these 10 countries has changed every year then we can examine how quickly they are adopting renewable sources of energy to meet their demands and how consumption of renewables compares to that of fossil fuels.

The tools here aren't just limited to the 10 countries listed above. It can be used for any country and even regions.
...
*Note: when comparing countries double check the values on the y-axis as it varies by region. If there is no data available for a region, an empty graph will be displayed.*
```{r}
